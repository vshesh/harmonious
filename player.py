...
def chord(root, symbol):
  return build_chord(
    [0, *default_voicing(symbol_layers.get(symbol, '5∆'))],
    int(root)
    if root.isnumeric()
...
def setter(notes):
  ...
  for line in stdin:
    notes[:] = chord(*line.split()) if len(line.split()) == 2 else notes
    print('notes = ', notes)


if __name__ == '__main__':
  manager = Manager()
  notes = manager.list([60, 65])
